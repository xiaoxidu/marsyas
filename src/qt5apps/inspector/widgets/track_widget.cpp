...
TrackWidget::TrackWidget( DebugController *debugger, QWidget * parent ):
  QWidget(parent),
  m_debugger(debugger),
...
void TrackWidget::setDisplayType( int type )
{
  if (type == Table)
  {
...
void TrackWidget::displayControl( MarSystem * system, const QString & path )
{
...
void TrackWidget::displayPort( MarSystem * system,
                                 FlowDirection port )
{
...
void TrackWidget::refresh(bool doAutoScale)
{
  if (m_path.isEmpty()){
    return;
  }
...
void TrackWidget::refreshFromControl()
{
  Q_ASSERT(m_system);
  ...
  if (control.isInvalid()) {
    qWarning() << "TrackWidget: invalid control path:" << m_path;
    clearPlot();
    return;
  }

  //if (!control->hasType<mrs_realvec>()) {
  //  qWarning() << "TrackWidget: control type not 'mrs_realvec':" << m_path;
  //  clearPlot();
  //  return;
  //}
...
void TrackWidget::refreshFromPort()
{
  Q_ASSERT(m_system);
  Q_ASSERT(!m_path.isEmpty());
...
void RealvecPlotCurve::setType(int type )
{
  switch(type)
  {
  case TrackWidget::Points:
    m_style = QwtPlotCurve::Dots;
    break;
  case TrackWidget::Sticks:
    m_style = QwtPlotCurve::Sticks;
    break;
  case TrackWidget::Line:
    m_style = QwtPlotCurve::Lines;
    break;
  case TrackWidget::Curve:
    m_style = QwtPlotCurve::Lines;
    m_fitted = true;
    break;
  default:
